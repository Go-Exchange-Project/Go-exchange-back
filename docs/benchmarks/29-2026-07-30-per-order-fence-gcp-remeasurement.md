...
- **시작 정렬**: 배리어 `LOAD_START_AT_MS=1785419460.638`. 양쪽 첫 non-zero VU 절대시각
  ...
  > **[30번에서 정정]** 이 0.201초는 `k6 프로세스 시작 시각 + progress 라인 elapsed`로 계산한
  > 값인데, **k6의 elapsed는 프로세스 시작이 아니라 초기화 이후부터 센다.** 30번에서 같은 계산이
  > 배리어보다 이른 시각(물리적으로 불가능)을 내놓아 드러났다 — load-gen 두 대의 `ulimit -n`이
  > 달라(1,048,576 vs 262,144) k6 init 시간이 달랐던 것이다. **정렬의 실제 근거는 배리어**
  > (공통 절대 epoch까지 `sleep`, 미달 시 throw — 이 실행도 throw 없음)**와 VM 시계 차이**이며,
  > 30번에서 측정한 두 load-gen의 시계 차이는 ≤0.04초였다. **이 "0.201초"를 skew 실측치로
  > 인용하지 말 것.** 결론(양쪽 정렬됨)은 바뀌지 않는다.
