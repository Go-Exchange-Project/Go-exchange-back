...
- "Postgres를 별도 인스턴스로 분리해 백엔드의 CPU 경합 문제(90%→30%)를 실측으로 해소했지만, 분리된 Postgres 인스턴스 자체가 CPU 부족(2 vCPU에서 389% 사용, load average 12.86)에 빠지면서 전체 처리량은 오히려 19% 감소하는 트레이드오프를 확인했다 — '병목을 없앤 것'이 아니라 '병목을 옮긴 것'이었음을 정량적으로 규명하고, 다음 단계(DB 인스턴스 CPU 증설 또는 원복)를 근거를 가지고 판단할 수 있게 됐다."
