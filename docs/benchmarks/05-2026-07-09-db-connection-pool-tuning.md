...
- "GCP `e2-medium`(2 vCPU) 단일 인스턴스에서 pprof로 CPU 프로파일링을 수행해, 매칭엔진이 아니라 미설정된 DB 커넥션 풀이 CPU의 37.9%를 SCRAM-SHA-256 재인증에 소모하고 있음을 발견했다."
- "커넥션 풀 설정(`SetMaxOpenConns`/`SetMaxIdleConns`) 변경만으로, 동일한 VM 사양에서 k6 스트레스 테스트가 완주 가능한 동시 사용자 수를 약 250명에서 800명(전체 시나리오 완주)으로, 병목이 나타나는 지점을 최소 4배 끌어올렸다."
- "같은 부하(VU 150 부근)에서 API 응답 p95 지연시간을 9.09초에서 4.30초로(약 53%) 줄였고, 처리량을 2.4배 늘렸다."
