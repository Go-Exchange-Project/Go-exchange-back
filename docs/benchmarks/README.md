...
- [36-2026-08-29-order-idempotency-key.md](36-2026-08-29-order-idempotency-key.md) — 36번째 테스트: 주문 생성 멱등성 키(B-2) 도입 후 500 VU 1회 측정. 주문·키·`ORDER_HOLD`·k6 iteration이 **414,447로 정확히 일치**(중복·유실 0), 가용성·업무 성공 100%, 1초 초과 0건, 정합성 4종 0. `POST /orders` p95는 35번 재구성치 33.2ms 대비 **48.9ms**로 기록하되 **사전 등록된 정량 게이트가 없어 회귀 판정은 하지 않는다**. 인덱스·WAL은 기준선으로만 남기고 멱등성에 귀속하지 않는다 (측정 SHA `21a80fe`)

> **목록 공백**: 19~35번 항목은 이 목록에 추가되지 않은 채로 남아 있다. 파일은 이 디렉터리에 있으므로 파일명 정렬로 찾을 수 있다. 이번 변경에서 그 공백을 소급해 채우지는 않았다.

> **진단 하니스 제거**: 33·34에서 쓴 정산 진단 하니스(`internal/service/settlement_diagnostic*_test.go`)는 복식부기 원장 전환(Task 5)에서 제거했다. 측정하던 지갑·단식원장 데이터 모델이 사라져 기존 기준선과 비교가 불가능하기 때문이다. 코드는 `37dd914` 이전 커밋에 있다.
