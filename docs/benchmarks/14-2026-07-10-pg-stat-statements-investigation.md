...
- "`pg_stat_statements`로 Postgres CPU 포화의 원인을 정량 분석해, 특정 느린 쿼리가 아니라 정산 트랜잭션당 15회에 달하는 쿼리 왕복 구조가 고빈도 체결 상황에서 누적 실행시간(10분간 895초, 2코어 용량의 71%)을 만들어낸다는 걸 규명했다. 이를 근거로 '쿼리 리팩터링'과 '인스턴스 증설' 중 리스크 대비 효과가 더 큰 방향을 판단할 수 있는 데이터를 확보했다."
