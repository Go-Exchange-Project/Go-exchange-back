...
- "DB 커넥션 풀 튜닝, 정산 워커 풀 확장 등 코드 레벨 최적화를 순서대로 적용한 뒤에도 CPU가 99% 이상 포화되는 것을 확인하고, VM 코어 수를 2배(2→4 vCPU)로 늘리는 대조군 실험을 진행했다. 결과, 처리량이 2.9배, p95 응답시간이 78% 개선되며 CPU가 진짜 병목이었음을 정량적으로 확인했다 — 무작정 사양을 올린 게 아니라, 코드 레벨 최적화를 먼저 소진한 뒤 실측 근거를 가지고 인프라 확장을 결정했다."
