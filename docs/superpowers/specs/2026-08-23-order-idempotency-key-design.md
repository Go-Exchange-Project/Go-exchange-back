...
- **decimal은 JSON 숫자나 부동소수점이 아니라 정규화된 문자열**로 넣는다
...
follower의 응답은 **레코드의 `outcome`을 읽어** 결정한다(§3.2). owner가 아직 제출 중이면
`PENDING` → **202**다. follower가 owner를 기다리지 않는다 — 기다리면 배치 지연이
요청 지연으로 전파되고, 202가 그 상태를 정직하게 표현한다.
...
CREATE TABLE IF NOT EXISTS order_idempotency_keys (
    ...
    order_id            BIGINT,
    outcome             TEXT,       -- PENDING | ACCEPTED | REJECTED | UNKNOWN
...
- `order_id`·`outcome`은 **nullable**이다. §4.2의 1단계에서는 아직 모른다
...
| 6c | 지문 **버전** — v1 저장 키는 v1 규칙으로 비교 | v2 도입을 흉내낸 테스트 |
