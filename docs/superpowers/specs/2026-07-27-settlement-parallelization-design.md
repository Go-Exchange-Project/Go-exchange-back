...
- **`AvgBuyPrice`는 순서 독립이 아니다(실측 확인)**: `balance.go:172-173`은 매 체결마다
  ...
  - 평균가는 케이스에 따라 **다름** — 기존 보유분 + 3건 케이스에서 정순 `50000011` vs 역순
    `50000010.9999999999999999`, **차이 1e-16**(이는 **관측된 한 반례**일 뿐 일반 상한이 아니다 —
    상한은 구현 후 순열 테스트로 측정한다).
...
### 결정 (A′) — 내부 미세 차이 허용 + 외부 표현·허용오차 명시

**병렬 정산으로 인한 `AvgBuyPrice`의 유한 정밀도 마지막 자리 차이는 허용한다.** `AvgBuyPrice`는
**표시·미실현 손익용 파생 상태**이며 주문 승인·홀드·체결 금액·출금 가능 잔액 계산, 원장 총량,
리컨실리에이션 판정 **어디에도 사용되지 않는다**. 단, **외부 표시는 포맷하고 테스트는 정확 일치
대신 허용오차로 비교한다.**

conflict-aware scheduling(B)을 택하지 않는 이유: 동일 지갑 충돌을 막으면 이번 병렬화 효과를 크게
제한할 수 있고, 별도 동시성 기계를 추가하게 되며, 보호되는 값은 금전 이동에 쓰이지 않는 파생 상태다.

**반드시 동일해야 하는 것**
- 지갑 `available`/`locked` 잔액, 자산 수량
- 주문 `FilledAmount`·상태
- 거래·outbox **멱등성**
- **자산 총량 보존 및 원장-지갑 불변식**(리컨실리에이션 판정)

**달라도 되는 것**
- 원장 행의 **커밋 순서**, 행별 중간 `BalanceAfter`
- `AvgBuyPrice`의 **유한 정밀도 마지막 자리**

**허용 범위는 숫자로 고정하되, 지금 단정하지 않는다**
- 현재 실험은 **한 반례에서 `1e-16` 차이를 관측**했을 뿐이다 — **"항상 1e-16"이라고 문서화하지
  않는다.** 일반 오차 상한은 **구현 후 순열 기반 테스트**(다양한 체결 수·가격·수량 순열을 반복해
  최대 오차 측정)로 정한다.
- 측정된 최대 오차보다 **충분히 작은 비즈니스 표시 단위**에서 외부 표현을 포맷한다.

**외부 표현 회귀 방지(동반 변경, `Go-exchange-front`)**: `AuthPanel.tsx:278`이 현재
`{wallet.avg_buy_price}`를 **원문 그대로** 출력한다(바로 아래 평가액은 `formatKRWAmount` 사용).
평균매수가도 **시장 가격 정밀도에 맞게 포맷**해 `50000010.9999999999999999` 같은 값이 화면에
노출되지 않게 한다. 금전 이동 오류는 아니지만 사용자에겐 품질 문제로 보이므로 **회귀 방지 조건**에
포함한다(백엔드 저장값은 그대로 두고 표현만 포맷).
...
1. **등가성(정합성 핵심) — 무엇이 같아야 하는지 정확히**: 같은 이벤트 열을 병렬 N(예: 4)과 직렬
   ...
   - **동일할 필요 없음(명시적 비-기대)**: 원장 행별 `AvailableBalanceAfter`/`LockedBalanceAfter`
     (커밋 순서 의존), `AvgBuyPrice`의 최종 자리.
   - 원장 행 집합을 **byte-for-byte 비교하지 않는다**.
   - `AvgBuyPrice`는 `Equal()`이 아니라 **합의된 tolerance 이하** 로 비교한다(아래 1-b).

1-b. **`AvgBuyPrice` 오차 상한 측정 + 외부 비노출**(A′ 조건):
   - **순열 기반 테스트**: 다양한 체결 수·가격·수량 조합의 **여러 순열**을 반복 적용해 최종
     `AvgBuyPrice`의 **최대 오차를 측정**하고, 그 값을 문서에 기록한다(“항상 1e-16” 식 단정 금지).
   - **tolerance 비교**: 병렬 결과와 직렬 결과의 차이가 **합의한 tolerance 이하**임을 단언.
   - **표현 테스트**: 포맷된 화면/API 표현에서는 그 차이가 **보이지 않음**을 단언
     (`Go-exchange-front` `AuthPanel.tsx` 평균매수가 포맷 — 동반 변경).
