...
| **H2-5000 큰 sweep** | maker 5,000건 소진 taker **묶음 측정**(§8.5) | `sweep_batch_mean_ns` | **C3** |
...
`measurement_invalid` 판정 조건(H2-5000·H4):

- `expected_trades_total != batch_size * 5000`
- `order_done_trades_total != expected_trades_total`
- `measured_trade_emits_total != expected_trades_total`
- `remaining_amount_total != 0`
- `sweep_batch_total_ns <= 0` 또는 `sweep_batch_mean_ns <= 0`
- 필수 작업량 필드 누락, `measurement_schema_version` 불일치

baseline의 censored는 "고치려는 문제의 크기"이므로 보존·보고한다. `measurement_invalid`는
그런 성격이 아니다 — 측정 자체가 성립하지 않았다는 뜻이므로 집계 전에 실패시킨다.

### 8.4 단일 sweep C3를 폐기한 이유

**측정 도구가 기존 기준을 구분하지 못한다는 것이 실측으로 확인됐다.**

작업량을 5,000으로 완전히 고정한(고정 UserID) 진단 20회의 실행 순서 값:

```
5.785 5.861 7.335 4.099 4.142 | 4.635 4.718 4.020 4.505 5.000
3.999 4.000 4.001 5.634 4.204 | 4.156 4.112 4.618 4.720 4.614   (ms)
```

전체 중앙값 4.559ms. 여기서 k개를 뽑는 모든 조합의 중앙값 분포:
...
실행 순서 5개씩 네 그룹의 중앙값은 전체 대비 **+26.9% / +1.7% / −12.2% / +1.2%** 로,
절반이 ±5%를 벗어난다.

C3의 허용폭은 ±5%다. **5회 중앙값의 오차가 ±9~11%이므로 이 도구로는 5% 차이를 구분할 수
없다.** 실제로 후보 `m64-c8`이 C3를 17.3µs(0.32%) 차이로 탈락한 적이 있는데, 그 판정은
오차 안에 완전히 묻힌다.

그룹 간 차이는 **측정 불안정성으로만 기록한다.** 워밍업이나 CPU 주파수 변화로 단정하지 않는다 —
그 원인을 규명한 관측이 없다.

따라서 **단일 sweep을 재는 C3 측정 단위를 폐기한다.** 이전 `sweep_total_ns`는 C3 입력으로
쓰지 않는다.

### 8.5 새 C3 측정 계약 — 묶음 평균과 같은 실행 파일의 대조군

**(1) 같은 실행 파일 안에서 대조군과 후보를 비교한다.**

이전 계측 전용 SHA와 quantum SHA를 직접 비교하지 않는다. 시간상 멀리 떨어진 두 실행을 비교하면
실행 환경 변화가 그대로 차이로 들어온다.

대조군은 **현재 quantum 구현에서 경계를 사실상 만나지 않게 한 테스트 전용 설정**이다.

```
maxMatchesPerTurn     = 1,000,000
maxConsecutiveCancels = 1,000,000
```

대조군 측정에서는 **모든 시나리오의 quantum yield가 0인지 단언한다.** 0이 아니면 대조군 측정은
실패다. 이 단언이 있어야 "대조군은 조각화가 발동하지 않은 상태"라는 전제가 관측으로 뒷받침된다.

이렇게 하면 대조군과 후보가 **같은 코드·같은 계측·같은 실행 환경**을 쓰고 설정값만 달라진다.

**(2) H2-5000을 묶음 측정으로 바꾼다.**

한 번의 기록값은 단일 sweep이 아니라 여러 sweep을 연속 실행한 **평균**이다.

| 항목 | 값 |
|---|---|
| 워밍업 | 8 sweep, 기록하지 않음 |
| 첫 측정 묶음 크기 | 64 sweep |
| 각 sweep | 정확히 5,000 trades |
| UserID | maker·taker·victim이 서로 겹치지 않는 고정값 |
| 준비 주문 | 측정 시작 **전에** 모두 넣고 완료 장벽 통과 |
| 타이머 시작 | 첫 측정 taker를 보내기 직전 |
| sweep 진행 조건 | taker 완료와 victim 취소 완료를 **모두** 확인한 뒤 다음 sweep |
| 타이머 종료 | 마지막 측정 sweep 완료 뒤 |

새 결과 필드: `sweep_batch_size`, `sweep_batch_total_ns`, `sweep_batch_mean_ns`,
`expected_trades_total`, `order_done_trades_total`, `measured_trade_emits_total`,
`remaining_amount_total`, `measurement_schema_version`.

**C3는 `sweep_batch_mean_ns`를 쓴다.**

**(3) 결과 형식 버전.**

`measurement_schema_version`을 올린다. 집계기는 아래를 **거부**한다.

- 버전 필드가 없는 이전 결과
- 이전 schema version
- 묶음 크기가 다른 결과의 혼합
- 필수 작업량 필드 누락
- `measurement_invalid`가 포함된 결과
- H2-5000·H4의 `0ns`

기존 JSON이 새 선택에 섞이는 경로를 코드로 막는다. 이전
`baseline`/`explore`/`confirm` 산출물은 **파일만 보존하고 어떤 선택에도 쓰지 않는다.**

### 8.6 정밀도 시험 — 전체 격자 전에 먼저

측정 도구가 C3의 5% 차이를 구분할 수 있는지 먼저 확인한다. 이것을 통과하기 전에는 후보를
선택하지 않는다.

대상은 **대조군**과 **가장 자주 양보하는 후보 `m16-c8`** 둘뿐이다.

- 묶음 크기 64에서 동일 seed로 **5쌍** 실행
- 실행 순서를 번갈아 바꾼다: 홀수 쌍은 대조군→후보, 짝수 쌍은 후보→대조군
- 각 쌍에서 `candidate_batch_mean_ns / control_batch_mean_ns` 비율을 계산
- 5개 비율의 중앙값을 구하고, 각 값이 중앙값에서 벗어난 **최대 비율**을 계산

**PASS 조건:** 모든 묶음이 유효하고, 최대 편차가 중앙값 대비 **2.5% 이하**.

**FAIL이면 묶음 크기를 128로 한 번만 늘려** 새 자료로 같은 시험을 반복한다.
**128에서도 FAIL이면 측정을 중단하고 보고한다.** 묶음 크기를 계속 늘리거나 5% 기준을 바꾸지
않는다.

### 8.7 산출물

각 회차는 JSON으로 저장한다. 디렉터리는 `precision/`, `explore-v2/`, `confirm-v2/`를 쓴다 —
이전 산출물을 덮어쓰지 않는다.

H0·H1·H2-1의 baseline(C1·C2의 기준값)은 종전대로 시나리오당 3회 수집해 보존한다.
계측 오버헤드는 `engine_bench_test.go` 전후 비교로 기록한다.
...
| 단계 | 격자 | 반복 | 목적 |
|---|---|---|---|
| 정밀도 시험 | 대조군 + `m16-c8` | 5쌍 (§8.6) | 도구가 5%를 구분하는지 |
| 1차 탐색 | `maxMatchesPerTurn ∈ {16, 64, 128}` × `maxConsecutiveCancels ∈ {8, 32}` = **6조합** | **3쌍** | 후보 좁히기 |
| 확증 | 1차 통과 조합 중 §9.4 규칙 상위 **2개** | **5쌍** | 최종 판정 |

**모든 후보는 바로 옆의 대조군과 짝지어 실행한다.** 각 쌍은 같은 seed를 쓰고, 홀수 반복은
대조군→후보, 짝수 반복은 후보→대조군으로 순서를 번갈아 바꾼다. 시간상 멀리 떨어진 baseline과
비교하지 않는다 — 실행 환경 변화의 영향을 줄이기 위해서다.

**최종 선택은 확증 측정에서 C1~C6을 통과한 조합만 대상으로 한다.** 1차 통과는 확증 대상을
고르는 데만 쓰고 판정 근거로 쓰지 않는다.
...
env 오버라이드로 준다. 파싱·주입 규칙은 §2.2.1·§2.2.2를 따른다.

### 9.2 절대 임계값을 지금 못 박지 않는 이유

baseline이 나오기 전에 절대 숫자를 적으면 그것은 근거가 아니라 사후 정당화다. 대신 **판정식과
선택 규칙을 지금 등록**하고, 숫자는 baseline에서 기계적으로 유도한다.

C1·C2의 상한에는 **하한 바닥(floor)** 을 둔다. control p99가 마이크로초 단위면 `×3`이
비현실적으로 촘촘해져 전 후보가 탈락하기 때문이다.

```
상한 = max(control_p99 × 3, snapshotInterval × 3)
     = max(control_p99 × 3, 300ms)
```

`snapshotInterval × 3`을 바닥으로 쓰는 이유: 이 파이프라인이 이미 감수하는 최소 관측 지연
단위가 스냅샷 코얼레싱 주기(100ms)이므로, 그보다 촘촘한 요구는 다른 부분과 정합하지 않는다.
...
| **C3** | **처리량 보존** | **각 쌍의 `candidate_batch_mean_ns / control_batch_mean_ns` 비율, 그 중앙값 ≤ 1.05** | 게이트 |
| C4 | 스냅샷 신선도 | H4 최대 간격 ≤ 300ms | 게이트 |
| C5 | censored 부재 | **후보** 측정의 전 시나리오 합계 censored **0건** | 게이트 |
| C6 | 의미 교차 검증 | B′ 의미 테스트 전부 통과 | 게이트 |
| C7 | yield 비용 | `quantum_yields_total` / 주문 수가 sweep 크기에 선형 이하 | 관찰 |

C3만 바뀌었다. **C1·C2·C4·C5·C6의 의미와 제한은 그대로다.**
중앙값의 반복 수는 단계에 따라 다르다 — 1차 탐색은 3쌍, 확증은 5쌍(§9.1).

`measurement_invalid`나 `censored`가 하나라도 있는 run-set은 집계와 후보 선택을 **즉시 실패**시킨다.

**개별 회차가 상한을 넘었으나 중앙값이 통과한 경우, 통과로 판정하되 보고서의 "한계" 절에
회차별 값과 초과 폭을 그대로 기록한다.** 숨기지 않는다.

35번 벤치마크의 cancel-command p99 비교는 **report-only**다. 판정에 쓰지 않는다.

### 9.4 선택 규칙 (사전 등록, 순수 함수)

C1~C6을 모두 통과한 조합 중:

1. `maxMatchesPerTurn`이 **가장 큰** 값 (처리량·emit 오버헤드 보존 우선)
2. 동률이면 `maxConsecutiveCancels`가 **가장 작은** 값 (주문 기아 여유 우선)
3. 그래도 동률이면 격자 순서상 먼저 오는 값

이 계산은 **순수 함수로 구현하고 단위 테스트로 고정한다.** 상위 2개 선정과 최종 선택이 같은
규칙을 써야 탐색이 결과를 편향시키지 않는다.

단위 테스트는 합성 표로 **세 개만** 고정한다: (1) 통과 후보 선택과 동률 규칙, (2) censored·
semantic 실패 탈락, (3) 통과 후보 0개 → 에러.

**집계기는 판정 전에 run-set을 정확히 한 번 검증한다.** censored 조기 반환보다 **먼저**
수행한다 — censored를 이유로 일찍 빠져나가면 run-set이 깨진 것을 영영 못 본다.

| # | 검증 |
|---|---|
| V1 | 필요한 각 시나리오가 **정확한 반복 수**로 존재 |
| V2 | 모든 JSON의 설정값이 디렉터리의 후보 설정과 일치 |
| V3 | 시나리오 안에 **중복 seed 없음** |
| V4 | **H4가 누락되면 에러.** max gap = 0으로 조용히 통과하지 않는다 |
| V5 | baseline도 후보 선택 **전에** 실제 parser로 읽어 V1~V3을 통과 |
| **V6** | **`measurement_schema_version`이 현재 버전과 일치. 이전 결과·필드 누락·묶음 크기 혼합·`measurement_invalid` 포함·H2-5000/H4의 0ns를 거부** |

**통과 조합이 0개면 P1 중단이다.** 임계값을 완화해서 통과시키지 않는다.
