...
H1은 기아를 재현하는 시나리오다. **수정 전 baseline에서는 주문이 영원히 admit되지 않을 수
있다.** 이 경우 분위수는 정의되지 않는다. 표본을 조용히 버리면 baseline이 실제보다 좋아 보이고,
전후 비교가 통째로 무의미해진다.

| 규칙 | 내용 |
|---|---|
| watchdog | 시나리오별 deadline(H1 기본 30s)을 넘긴 표본은 **censored(∞)** 로 기록한다 |
| 버리지 않는다 | censored 표본을 drop하지 않는다. 개수를 별도 보고한다 |
| 분위수 | censored가 1건이라도 있으면 해당 회차의 p99를 `+Inf`로 취급한다 |
| 판정 | censored가 1건이라도 있는 후보는 해당 기준 **자동 탈락** |
| baseline | baseline의 censored 개수는 "고치려는 문제의 크기" 자체이므로 반드시 보고서에 남긴다 |
...
이 네 개가 없으면 4단계의 전후 비교는 사후 재구성이 되어 근거로서의 가치를 잃는다.

산출물 경로는 `_workspace/quantum/` (중간 산출물 규약).
...
단위가 스냅샷 코얼레싱 주기(100ms)이므로, 그보다 촘촘한 요구는 시스템의 다른 부분과 정합하지
않는다.
...
**settlement dispatch wait 기반 교차 검증은 제거했다.** 두 큐는 부하 조건도 단위도 다르고,
로컬 하니스에는 settlement 경로 자체가 없어 비교 대상이 성립하지 않는다. 그 자리를 C6이
대신한다 — quantum이 취소 의미를 깨지 않았다는 것은 지연 수치가 아니라 **B-1~B-5의 의미
테스트**로 확인하는 것이 옳다.
...
이 계산은 **순수 함수로 구현하고 단위 테스트로 고정한다.**

```go
// candidates는 측정 결과. 판정과 선택에 부작용이 없다.
