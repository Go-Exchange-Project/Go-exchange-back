...
`available_balance - ledger_available_sum`(gap)이 `implied_initial_available`과
정확히 일치하면(레거시 초기 잔액이 gap을 전부 설명하면) `check="legacy_mismatch"`로,
아니면 `check="ledger_wallet"`(진짜 버그)로 분류한다 — 레거시와 진짜 버그가 같은
지갑에 겹친 경우 gap이 어긋나므로 정확히 `ledger_wallet`로 잡힌다. 스트레스 테스트
