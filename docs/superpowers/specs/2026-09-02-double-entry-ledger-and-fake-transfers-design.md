...
- 프론트엔드 변경(§13.5에서 범위만 표시) — **정정(계획서 Task 7, 조사 D):** 원장 전환으로
  인한 기존 프런트 수정은 실제로 불필요했다(`WalletResponse` 6개 필드 그대로 유지, §13.5).
  다만 계획서는 이 설계에 없던 **가짜 입출금 체험용 신규 화면(`/assets`)**을 Task 7 범위로
  별도 추가했다 — "프론트엔드 변경 없음"이 아니라 "원장 전환 자체는 프런트를 건드리지 않되,
  Task 6이 추가한 새 기능(입출금)의 화면은 새로 만든다"로 좁혀진 것이다.
- GCP 측정
