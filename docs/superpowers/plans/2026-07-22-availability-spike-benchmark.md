...
- [x] **Step 1: 도구 디스패치 지연 회피 검증**(22번 교훈 #4) — gcloud ssh 스냅샷을
...
- [x] **Step 2: VM 상태·A/B 커밋 확인** — 서버·DB·load-gen VM이 e2-highcpu-4/standard-8로
  떠 있는지, `git log -1 c899284`·현 main HEAD 확인. VM이 stop 상태면 start(같은 머신 타입).
- [x] **Step 3: k6 스파이크 스크립트 작성 + 리포 커밋** — `_workspace/loadtest/order-spike-availability.js`
...
- [x] **Step 1: 배포 A** — `git archive c899284` → `scp` → `sudo tar` 추출 → 이전 bench `.env`
  복사 → `sudo docker compose --project-directory <bench-dir> -f docker-compose.stress.yml
  up -d --build backend`. 기동 로그로 커밋/구성 확인.
- [x] **Step 2: 리셋** — 9테이블 `TRUNCATE ... RESTART IDENTITY CASCADE` + backend 재시작 +
  기동 로그 `bootstrap loaded=0` 확인.
- [x] **Step 3: A 런** — `k6 run -e BASE_URL=http://$SRV:8080 -e DEV_TOOLS_TOKEN=$TOKEN
...
- [x] **Step 4: 드레인 대기 + 정합성 5검사** — execution 채널·정산 워커 큐 0 확인 후:
...
- [x] **Step 5: 수집** — k6 요약(custom 포함)·게이지 샘플·CPU·`RestartCount`
...
- [x] **Step 1: 리셋 + 배포 B** — 리셋(9테이블 TRUNCATE + 재시작) → `git archive`(현 main HEAD)
  → 배포(Phase 1 Step 1과 동일 절차) → 리셋 재확인 `bootstrap loaded=0`.
- [x] **Step 2: B 런** — Phase 1 Step 3과 **동일 스크립트·동일 샘플링**. 추가로 런 종료 직전
  `/metrics`에서 `orders_admission_rejected_total{stage="engine_gate"|"engine_handoff"|"coordinator"}`
  를 읽어 셰딩 분포 기록(리셋 전에).
- [x] **Step 3: 드레인 + 정합성 5검사** — Phase 1 Step 4와 동일.
- [x] **Step 4: 수집** — Phase 1 Step 5와 동일 + 셰딩 분포. pprof 30초는 best-effort(`& disown`).
- [x] **Step 5: VM stop** — 서버·DB·load-gen 전부 stop(측정 종료).

---

### Phase 3: 분석 + 문서 + README + 메모리

- [x] **Step 1: A/B 판정표 작성** — 스펙의 성공 기준 7항목을 A/B 실측으로 채운다:
...
- [x] **Step 2: 벤치마크 문서** — `docs/benchmarks/23-<측정일>-availability-spike-ab.md`
...
- [x] **Step 3: 완료 문서 + README** — `docs/refactor/15_2차⑤_가용성_스파이크_실증_완료.md`
  (왜/어떻게/결과 — ①~④의 미뤄둔 수치가 채워졌음을 명기) + README 2차 표 ⑤ 🔲→✅ +
  "2차 리팩토링(가용성 100%) 완결" 표기.
- [x] **Step 4: 메모리 정정** — `goexchange-performance-goal` 메모리의 VM 클래스
  ("고정 e2-medium/e2-small")가 실제 벤치마크(e2-highcpu-4)와 충돌 → 사용자와 프로덕션
  목표 하드웨어를 확인한 뒤 메모리를 사실과 일치시킨다.
- [x] **Step 5: Commit + 푸시 + CI** — 문서 커밋(author→reviewer), `gh run watch` 그린.
