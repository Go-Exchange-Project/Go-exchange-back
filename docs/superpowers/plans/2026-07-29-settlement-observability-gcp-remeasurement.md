...
토큰 교체는 2026-07-29 완료됐다. 교체 당시 외부 ingress를 차단한 상태에서 구 토큰 403 거부와
신규 토큰의 인증 통과를 확인했다. **실제 토큰 값과 fingerprint는 문서 및 아티팩트에 기록하지 않는다.**

전체 부하 실행 전:

- ingress를 열기 전에 현재 secret이 **비어 있지 않고 기대 길이**인지 확인한다.
- 서버와 승인된 호출자의 secret이 일치하는지는 **boolean 결과로만** 확인한다. **값이나 hash를
  출력하지 않는다.**
- 현재 토큰으로 **인증 계층을 통과하는지** 확인한다. 검증용 요청은 **잔고를 변경하지 않거나**
  일회성 사용자로 수행한다.
- 검증 실패 시 **전체 부하 실행을 금지**한다.
- **폐기된 구 토큰을 복원하거나 다시 사용하지 않는다.**
