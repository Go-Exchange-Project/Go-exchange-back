...
- [x] **Step 1: 코드 확인** — HEAD가 ②-수정(`57663b1` 이후)을 포함하는지, `cmd/settlement_pipeline.go`
  존재 확인. `git log --oneline -3`.
- [x] **Step 2: 기동(pprof on + 스윕 env 전달)** — `GOEXCHANGE_ENABLE_PPROF=true`로 기동.
  ...
  리뷰가 잡은 함정).
- [x] **Step 3: 도달 확인** — `curl http://127.0.0.1:6060/debug/pprof/` 200 +
  `/metrics`에 `settlement_worker_queue_length`·`matching_engine_channel_length` 노출 확인.
- [x] **Step 4: 기동 로그로 구성 확인** — `settlement partitions=<P> concurrency=<N>` 로그가 **의도한
...
- [x] **Step 1: 리셋** — 9테이블 `TRUNCATE ... RESTART IDENTITY CASCADE` + backend 재시작(해당 N으로)
  + 기동 로그 `bootstrap loaded=0` 및 `concurrency=N` 확인.
- [x] **Step 2: 부하** — `_workspace/loadtest/crossing-flood.js`를 24번과 **동일한 VU·지속시간**으로
  실행(24번 문서의 값을 그대로 따른다 — 비교 가능성이 핵심).
- [x] **Step 3: 안정 상태 샘플(로드 중반, 15초 간격 수 회)**
...
- [x] **Step 4: 반복 goroutine 덤프** — `/debug/pprof/goroutine?debug=2`를 안정 상태에서
  **10~15초 간격 ≥3회** 저장. 정산 worker들이 어디서 막혀 있는지(pgx Exec? 락 대기? jobs 수신 대기?)
  **지속 패턴**으로 판정(단일 스냅샷 단정 금지).
- [x] **Step 5: CPU pprof 30초** — `/debug/pprof/profile?seconds=30`.
- [x] **Step 6: 드레인 + 정합성 5검사** — 채널·큐 0 확인 후: 리컨실리에이션 게이지 2종(원장-지갑,
...
- [x] **Step 7: 기록** — 위 값을 N별 표로 저장(`_workspace/`).

---

### Phase 2: 기준선 게이트 (N=1)

- [x] **Step 1: 24번 재현 확인** — N=1 결과가 24번 진단과 **같은 그림**인지:
  정산 큐 1개가 256 고정 포화 · 나머지 0 · 처리율 24번의 **~350 trades/s** 근방.
- [x] **Step 2: 판정** — 재현되면 스윕을 신뢰하고 진행. **재현이 안 되면**(부하·환경이 달라졌다는 뜻)
...
- [x] **Step 1: 바인딩 해소 여부** — N이 커질 때 **정산 큐 깊이가 256 고정에서 풀리는지**.
  풀리면 "정산 워커 바인딩 해소" 확정.
- [x] **Step 2: 처리량 스케일링** — trades/s를 N=1 대비로 표기. **N에 비례하지 않을 수 있다**
  (동일 지갑/주문 행 락 직렬화 — 스펙이 예고한 한계). 비례하지 않으면 **어디서 꺾이는지**를 기록.
- [x] **Step 3: 새 바인딩 링크 식별** — 아래 중 무엇인지 goroutine 덤프·게이지·CPU로 판정:
...
- [x] **Step 4: 정합성 판정** — 모든 N에서 5검사 0. **하나라도 위반이면 그것이 최우선 결과**이며,
  병렬화 롤백(`CONCURRENCY=1`)을 즉시 권고하고 원인 규명으로 전환한다.
- [x] **Step 5: 권장 운영값** — 처리량·정합성·풀 여유를 종합해 **기본 `CONCURRENCY` 권장치**를 제시
...
- [x] **Step 1: 벤치마크 문서** — `docs/benchmarks/25-<측정일>-settlement-parallelization-remeasurement.md`:
...
- [x] **Step 2: 완료 문서 보강** — `docs/refactor/18_3차②_정산_병렬화_완료.md`의 "처리량 실증은
  재측정" 문장에 **이 문서 링크와 결과 한 줄**을 추가(미뤄둔 수치를 닫는다).
- [x] **Step 3: README** — 3차 표에 재측정 결과 반영 + **3차 리팩토링 완결** 표기(코드 ③①②-진단②-수정
  + 실증). 남은 병목은 백로그로 승격.
- [x] **Step 4: Commit + 푸시 + CI** — 문서 커밋(Conventional Commits·한글, 스킬 불가 시 직접 작성).
- [x] **Step 5: 정리** — GCP 썼으면 VM stop, 로컬이면 compose down.
