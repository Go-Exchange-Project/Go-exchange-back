...
> **⚠ 다음 GCP 측정 세션이 먼저 알아야 할 것 — preflight 명령을 바꿔야 한다.**
> 29~32번은 시작 전 `git diff 82b4d7f..HEAD -- '*.go'`가 **비어 있음**을 확인했다.
> 이 진단 하니스는 `internal/service/*_test.go` 2개를 추가하므로 **그 명령이 더 이상 비지 않는다.**
> 배포 바이너리는 무변경이다(`_test.go`는 `go build`에 들어가지 않는다). 확인하려면:
>
> ```bash
> git -C Go-exchange-back diff --stat 82b4d7f..HEAD -- '*.go' ':(exclude)*_test.go'
> ```
>
> **이쪽이 비어 있어야 한다.** 기존 명령의 출력만 보고 중단하지 않는다.
