...
| ② | **자금 홀드 배칭** | 동시 주문 홀드를 그룹커밋 코디네이터로 모아 한 트랜잭션에 persist+hold(B-4식) → DB 왕복 N:1 상각으로 천장을 목표 위로. 개별 잔고 부족은 배치 안에서 격리. 백로그 "주문 생성 경로 DB 왕복" 승격 | 🔨 설계 완료 ([스펙](../superpowers/specs/2026-07-19-order-hold-group-commit-design.md)) | — |
