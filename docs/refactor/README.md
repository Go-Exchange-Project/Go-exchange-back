...
**2. 매칭 quantum** (최악 fan-out에서의 진행성) — **코드·로컬 검증 완료, 처리량 미검증**
...
2. **축 2 — 매칭 quantum** — **코드·로컬 검증 완료.** 기본값 `maxMatchesPerTurn=128` /
   `maxConsecutiveCancels=8`. 취소 기아 방지 구조와 결정적 스케줄링 계약(5,000 체결당 yield 39)은
   검증됐고 C1·C2·C4는 300ms 안전 상한을 통과했다. **처리량 보존(±5%)은 로컬에서 판정하지
   못했다** — 측정 정밀도 부족이며 근본 원인은 미확정이다. **실제 처리량과 500 VU 회귀는 모놀리스
   정확성 작업이 모두 끝난 뒤 최종 통합 GCP 실행에서 확인한다.**
   ([설계](../superpowers/specs/2026-08-30-matching-quantum-design.md) ·
   [계획](../superpowers/plans/2026-08-30-matching-quantum.md))
...
**34번 — 인덱스 단독 수정·재측정(완료)**
([34](../benchmarks/34-2026-08-18-buy-order-index-remeasurement.md)): 33번이 지목한 원인만
고쳐 같은 조건으로 다시 쟀다. 운영 변경은 `trades(buy_order_id)` 인덱스 하나뿐이고, 32-B가 쓴
이미지 위에 **migration 006 한 레이어만 얹은 파생 이미지**로 실행해 바이너리를 바이트 단위로
보존했다(`/app/go-exchange-back` SHA-256 동일, 레이어 delta 1).

- **최종: 유한 실행으로 검증된 최고 용량은 여전히 합산 500 VU다.** 750은 r1 통과 / r2 실패로
  갈렸고, 사전 등록 분기대로 **평균 없이 탈락**시켰다.
- **로컬 6셀 × 2회 PASS** — `market_terminal` 크기 기울기가 **4.885/5.724 → 0.998/1.077**.
  12/12 셀에서 Seq Scan이 사라지고 인덱스를 탄다(**mid·full 8셀 + `initial/N1` 2셀은 Index Scan,
  `initial/N8` 2셀은 Bitmap Index Scan**). `full/N8` 기준 seq 튜플 읽기 85,049,132 → **0**,
  대상 쿼리 평균 115.16ms → **0.040ms**, DB 실행시간 비중 86.1% → **0.9%**.
- **GCP 500 PASS** — 전 SLI 100%, 셰딩 0, `job_growth 1.078`.
- **같은 750 VU에서 32-B 대비**: 업무 성공률 93.484% → 100%(r1) / 99.67%(r2),
  **셰딩 36,989 → 0 / 2,024(약 18배 감소)**, `job_growth` 2.58 → 1.256 / 1.462.
- **job mix는 32-B와 동일**(market/terminal 73.4~73.7%)이라 개선을 mix 이동으로 설명할 수 없다.
- **⚠ 500 VU의 32-B 시절 growth는 측정되지 않았다.** "인덱스가 2.58을 1.078로 만들었다"고 쓰지
...
### 주문 생성 멱등성 키 — B(정확성 부채) 2번 ([36](../benchmarks/36-2026-08-29-order-idempotency-key.md))

35번과 같은 종류의 부채다. 성능이 아니라 **계약 전환**이다. 응답을 받지 못한 주문 재시도가
중복 주문을 만들던 창을 닫았다.

- **닫은 결함**: 클라이언트가 `POST /orders` 응답을 받지 못하면(네트워크 오류·타임아웃) 서버가
  주문을 만들었는지 알 수 없다. 재시도하면 **같은 의도의 주문이 두 건** 생기고 hold도 두 번
  잡힌다. 사용자는 자기 자금이 두 번 묶인 것을 뒤늦게 본다.
- **바뀐 계약**: `POST /orders`가 `Idempotency-Key` 헤더를 **필수로 요구**한다(없으면 400).
  `(user_id, idempotency_key)` UNIQUE가 애플리케이션 lock 없이 직렬화하고, 같은 키의 재요청은
  저장된 결과를 그대로 돌려준다. 같은 키를 **다른 주문 내용**에 쓰면 409다.
- **outcome 네 상태**: `PENDING`(서버가 그 뒤를 durable하게 알지 못함) → 202,
  `ACCEPTED` → 200, `REJECTED`(보상 완료) → 503, `UNKNOWN`(보상 실패) → 503.
  `REJECTED`·`UNKNOWN`도 `order_id`를 함께 준다 — 사용자가 그 주문을 찾아갈 수 있어야 한다.
- **⚠ 사용자가 체감하는 대가**: `UNKNOWN`·`PENDING`은 **같은 키로 재시도해도 같은 상태**가
  돌아온다. 재시도가 해결 수단이 아니므로 주문 상태 조회로 안내한다.
- **관측**: `PENDING`은 프로세스가 hold 커밋 직후 죽으면 아무 counter도 오르지 않는다.
  그래서 `order_idempotency_stale_pending` **gauge**와 전용 monitor를 뒀다(30초 주기, 시작 직후
  1회 조회, 조회 실패 시 gauge를 0으로 덮지 않는다).
- **500 VU 10분 1회**: 주문 **414,447** : 멱등성 키 **414,447** : `ORDER_HOLD` **414,447** :
  k6 iteration **414,447**이 **정확히 일치**(중복 0, 유실 0, 키 없는 주문 0). 가용성·업무 성공
  **100%**, 1초 초과 **0건**, HTTP 실패 **0**, 정합성 4종 **0**.
  outcome은 전부 `ACCEPTED`이고 stale `PENDING` **0**, `UNKNOWN` **0**,
  outcome update 실패 **0**, monitor 오류 **0**.
- **새 기준선**(합격선 아님): `POST /orders` p50 30.7ms · **p95 48.9ms** · p99 85.1ms.
  35번 hold window를 같은 PromQL로 재구성한 값은 p95 33.2ms다.
- **⚠ p95로 회귀를 판정하지 않는다.** 이 지표에는 **사전 등록된 정량 임계값이 없고**,
  동일 구성에서도 p95 run-to-run 분산이 약 35%다. 판정하려면 임계값 등록과 반복 실행이
  필요하다 — 추가 유료 실행 사안이다.
- **⚠ 인덱스·WAL을 멱등성에 귀속하지 않는다.** 테이블 84 MB · UNIQUE 47 MB ·
  부분 인덱스 2,232 kB, WAL 증가 ≈3.16 GiB는 **시스템 전체 workload** 값이다.
  대조군 없는 단일 실행이라 순증분을 분리할 수 없다. 부분 인덱스 2 MB도 종료 시 PENDING 0건
  상태의 **churn 흔적**이지 정상 상태 크기가 아니다.
