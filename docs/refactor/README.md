...
**다음은 31번 — DB 인스턴스 4→8 vCPU급 증설, 2단계로 진행한다**: ① `N=4`+DB8로 **DB만 바꾼 안전
기준선**(N=4는 이미 worker 포화라 개선이 작아도 여기서 기각하지 않는다) → 안전 게이트 통과 시
② `N=8`+DB8로 **실제 가설 검증**(568건이 0으로 돌아오는지, 이득을 안전하게 회수하는지).
**vCPU와 함께 RAM도 늘어나므로 "CPU 단독"이 아니라 "인스턴스 크기" 실험이며**, 15번의 2.46배는
**가능성을 지지하는 선례일 뿐 예상 배율이 아니다.**
