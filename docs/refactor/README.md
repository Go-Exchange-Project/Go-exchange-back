...
| **신규 지갑 생성 시 비결정적 INSERT 순서로 인한 PostgreSQL 데드락 (가설)** | [25번 실측](../benchmarks/25-2026-07-27-settlement-parallelization-remeasurement.md): `CONCURRENCY=8`에서 데드락(SQLSTATE 40P01) 4~6건/런(N=4도 1건), goroutine 덤프가 `WalletRepository.CreateZeroBalanceWallets`를 지목. **가설**: `SettleTradeBatch`의 신규 지갑 생성 목록(`missingCreatable`, settlement_batch.go:162-167)이 **map 순회 순서를 그대로 사용**해 트랜잭션별 INSERT 순서가 달라진다(락 단계는 `sortedUintKeys`로 정렬하지만 **INSERT 단계만 정렬이 없다**) → 겹치는 신규 지갑 키를 동시 생성할 때 튜플 락 순환으로 데드락. **검증 절차**: 겹치는 신규 `WalletKey` 집합을 **역순으로 두 트랜잭션에서 동시 생성**해 `40P01`을 재현(RED) → `(UserID, CoinSymbol)` 정렬 후 `CreateZeroBalanceWallets` 호출 → 소멸 확인 + N=4 짧은 재측정에서 fallback 소멸 확인. **주의: 정렬은 이 INSERT 순서 데드락 클래스만 제거한다** — 수정 후에도 fallback이 남으면 SQLSTATE·goroutine/DB 로그로 **다른 락 경로를 재분류**해야 한다(안전망인 단건 폴백은 정상 작동했고 정합성 위반은 0) | **승격 권고(23번 재실행 선행)** — 알려진 동시성 이상을 안고 최종 사용자 관점 검증(23번)을 돌리면 해석이 흐려지므로, 이 짧은 수정 사이클을 먼저 닫는다 |
