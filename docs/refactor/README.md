...
| 5 | **B-1** | 스냅샷 코얼레싱(심볼별 100ms) + 스냅샷 캐시 + WS 심볼 구독. 급등락 시 엔진이 브로드캐스트에 결박되는 문제 해소 | 🔶 진행 중 — **B-1a(코얼레싱+캐시+REST 분리) 코드 완료**([설계](../superpowers/specs/2026-07-13-orderbook-snapshot-coalescing-design.md)), GCP 재측정·WS 구독(B-1b) 대기 | — |
