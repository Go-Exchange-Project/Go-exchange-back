...
| ②-수정 | **정산 병렬화** (순서 보존 브로드캐스트) | ②-진단이 확정한 정산 워커 바인딩 해소: dispatcher 1 + worker N으로 **정산 DB 작업은 병렬**, `batchSeq` + completion 채널 + 작은 reorder 버퍼로 **브로드캐스트는 디스패치 순서 직렬 방출**(프런트가 수신 순서로 삽입하므로 순서 흩뜨림은 가시 회귀 — `Index.tsx:294`). 종결 이벤트는 앞선 배치의 커밋·outbox·브로드캐스트 완료 배리어. 정산 로직·데드락 안전(오름차순 2단계 락) 무변경. `SETTLEMENT_WORKERS` 의미 변경(파티션 수→동시 정산 수) 명시, DB 풀(기본 25) 여유 내로 상한, 실증은 1/2/4/8 스윕 | 🔨 설계 완료 ([스펙](../superpowers/specs/2026-07-27-settlement-parallelization-design.md)) | — |
