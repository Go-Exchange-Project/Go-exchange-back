...
| A-4 | 취소-체결 레이스의 카운터파티 오더북 발산 — 주문 상태 전이 소유권을 엔진으로(시퀀서화) | **실증됨** — [22번 사전 작업 로컬 스모크](bug-findings-2026-07-17-cancel-fill-race-and-market-buy-overspend.md#버그-1-취소-체결-레이스-백로그-a-4의-첫-실증)에서 취소 154건 중 500 21건(13.6%), `failed_settlements` 24건 재현. 승격 권고 |
| **시장가 매수 반올림 오차** | 분할 체결 시 quote 소진액이 예산을 epsilon(≈1.88e-9) 초과 → 실패 기록(`failed_market_completions`)조차 CHECK 제약 위반으로 실패하는 이중 실패 | [발견 문서](bug-findings-2026-07-17-cancel-fill-race-and-market-buy-overspend.md#버그-2-시장가-매수-반올림-오차로-인한-이중-실패-신규-백로그-미등재) — 로컬 스모크(유저 20명, 55초)에서 서로 다른 주문 97건 재현. 승격 권고 |
