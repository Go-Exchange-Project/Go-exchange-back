...
| 7 | **B-1c** | 체결 WS 브로드캐스트를 정산 배치(B-4) 단위로 묶어 발행(`{"type":"trades","data":[...]}`). [19번 벤치마크](../benchmarks/19-2026-07-16-b1-b4-batch-remeasurement.md) 해석 2가 드러낸 fanout 병목의 후속 | ✅ 완료 (GCP 측정은 다중 심볼 프로파일 작업과 묶어 다음 사이클) | [7_B-1c_체결_브로드캐스트_배치화_완료.md](7_B-1c_체결_브로드캐스트_배치화_완료.md) |
| 8 | **B-3** | 매칭 엔진 심볼 샤딩(심볼 해시 → N개 엔진 goroutine). B-4 이후에 해야 효과 측정 가능 | 🔲 예정 | — |
