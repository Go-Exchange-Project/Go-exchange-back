...
| 항목 | 내용 | 승격 조건 |
|---|---|---|
| **OutboxWriter 처리율** | flush가 배치 상한 64에 포화(평균 54.4건/flush, ≈66ms/flush)돼 write-ahead 관문이 이벤트 처리율을 캡 — 배치 상한 상향 및/또는 심볼 파티션별 병렬 writer(순서 보장은 심볼 내만 필요) | **승격 권고 — [21번](../benchmarks/21-2026-07-17-b3-sharding-remeasurement.md)이 실측한 현재 병목(최우선)** |
...
| 기본 엔진 샤드 수 1로 하향 | 병목이 다운스트림일 때 샤드 N개 = 주문 버퍼 N×1024로 깊어져 p95만 악화(21번: 샤드8 p95 +57%) — 기본 1, env로 필요 시 확대 | 21번 후속 소품 |
