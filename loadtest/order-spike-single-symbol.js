...
// 22번 계획: 단일 심볼 집중 + 급등락 램프. BASE_PRICE는 2,000,000 초과 구간이라
// tick_size가 max_tick_size(1000)로 고정된다(market_rules.json tick_rules).
const BASE_PRICE = 50000000;
const TICK = 1000;
const ORDER_AMOUNT = '0.001';
const MARKET_BUY_QUOTE_AMOUNT = '50000'; // ORDER_AMOUNT(0.001 BTC) * BASE_PRICE 상당

const BUYER_KRW_FUNDING = '1000000000000';
const SELLER_BTC_FUNDING = '1000000';

const MAKER_RATIO = 0.6; // 메이커 60% / 테이커 40%
const MAKER_CANCEL_PROBABILITY = 0.3;
const TAKER_MARKET_RATIO = 0.5; // 테이커 중 시장가 50% / crossing 지정가 50%
...
// 급등락 모사: 평시 -> 30초 급등 -> 유지 -> 30초 급락 -> 회복 관찰.
// 로컬 스모크에서는 각 STAGEn_DURATION/STAGEn_VUS를 짧게 오버라이드한다.
const SPIKE_STAGES = [
  { duration: __ENV.STAGE1_DURATION || '2m', target: parseInt(__ENV.STAGE1_VUS || '300', 10) },
  { duration: __ENV.STAGE2_DURATION || '30s', target: parseInt(__ENV.STAGE2_VUS || '3000', 10) },
  { duration: __ENV.STAGE3_DURATION || '3m', target: parseInt(__ENV.STAGE3_VUS || '3000', 10) },
  { duration: __ENV.STAGE4_DURATION || '30s', target: parseInt(__ENV.STAGE4_VUS || '300', 10) },
  { duration: __ENV.STAGE5_DURATION || '2m', target: parseInt(__ENV.STAGE5_VUS || '300', 10) },
];
...
export function setup() {
  ...
  for (let batchStart = 1; batchStart <= TOTAL_USERS; batchStart += SETUP_BATCH_SIZE) {
    ...
    // 홀=매수자(KRW 충전) / 짝=매도자(BTC 충전) — 단일 심볼이라 페어링 불필요.
...
// (단일 고정가는 즉시 체결돼 쌓이지 않으므로 반드시 가격 분산).
function makerFlow(user) {
  ...
  sleep(1 + Math.random() * 2); // 제출 후 1~3초 뒤 취소 시도
...
// 테이커 시장가: MARKET BUY는 price=0/amount=0/quote_amount>0, MARKET SELL은
// amount>0/price=0/quote_amount=0 제약(internal/service/order_service_test.go 참조).
...
// 테이커 crossing 지정가: 메이커 최대 스프레드(±5틱)를 확실히 넘는 가격(±10틱)으로
// 즉시 체결을 노린다.
