...
export function setup() {
  ...
  for (let batchStart = 1; batchStart <= TOTAL_USERS; batchStart += SETUP_BATCH_SIZE) {
    ...
    const fundRequests = batchIndices.map((i) => {
      ...
      // 사용자당 fund 호출은 하나뿐이지만(매수자는 KRW, 매도자는 자기 심볼 하나만
      // 받는다), 심볼별로 잔고를 나눠 보는 검증이 쉽도록 키에도 심볼을 넣는다.
      const fundBody =
        role === 'buyer'
          ? { coin_symbol: 'KRW', amount: BUYER_KRW_FUNDING, request_key: `loadtest-fund-${FUND_RUN_ID}-${i}-KRW` }
          : { coin_symbol: symbol, amount: SELLER_COIN_FUNDING, request_key: `loadtest-fund-${FUND_RUN_ID}-${i}-${symbol}` };
      return [
        'POST',
