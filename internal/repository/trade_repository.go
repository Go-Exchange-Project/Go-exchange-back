...
func (r *TradeRepository) ListByUserID(userID uint, filter TradeListFilter) ([]UserTrade, error) {
	...
	query := r.DB.Table("trades").
		Select(
			"trades.id, trades.idempotency_key, trades.coin_symbol, trades.price, trades.quantity, trades.traded_at, trades.buy_order_id, trades.sell_order_id, CASE WHEN buy_orders.user_id = ? THEN ? ELSE ? END AS side",
			userID,
			model.OrderSideBuy,
			model.OrderSideSell,
		).
