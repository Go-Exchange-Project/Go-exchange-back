...
func (c *HoldCoordinator) HoldBatch(reqs []holdRequest) ([]holdResult, error) {
	...
	err := c.DB.Transaction(func(tx *gorm.DB) error {
		...
		//    쥐고 있으므로 대기가 없다.
		//
		//    available뿐 아니라 짝이 되는 locked까지 여기서 잠근다. 잠금 분개는
		//    available에서 빼고 locked에 더하므로 Record는 두 계정을 모두 잠근다 —
		//    locked를 빼면 그 계정만 루프 중간에 처음 잠기게 되어, 트랜잭션 전체의
		//    획득 순서가 오름차순에서 벗어난다.
		accountRepo := c.Ledger.Accounts.WithTx(tx)
		specs := make([]repository.AccountSpec, 0, len(activeOwners)*2)
		for _, i := range activeOwners {
			order := reqs[i].order
			userID := order.UserID
			specs = append(specs,
				repository.AccountSpec{
					AccountType: model.AccountUserAvailable,
					OwnerUserID: &userID,
					Asset:       orderHoldAsset(order),
				},
				repository.AccountSpec{
					AccountType: model.AccountUserLocked,
					OwnerUserID: &userID,
					Asset:       orderHoldAsset(order),
				},
			)
		...
		// accountKey(값 타입)로 찾는다. repository.AccountSpec을 map 키로 쓰면 안 된다 —
		// OwnerUserID가 포인터라 값이 같아도 주소가 다르면 다른 키가 된다(ledger_service.go의
		// 같은 경고 참고). 그러면 아래 조회가 매번 실패해 잔고와 무관하게 전부 거절된다.
		accountIDByKey := map[accountKey]uint{}
		...
		// 잔고 사전 검증은 available 계정만 본다. 잠근 계정에는 locked도 섞여 있으므로
		// 걸러내지 않으면 잠긴 잔액을 쓸 수 있는 잔액으로 오해할 수 있다.
		isAvailableAccount := make(map[uint]bool, len(accounts))
		for _, account := range accounts {
			if account.AccountType == model.AccountUserAvailable {
				isAvailableAccount[account.ID] = true
			}
		}
		available := make(map[uint]decimal.Decimal, len(balances))
		for _, balance := range balances {
			if !isAvailableAccount[balance.AccountID] {
				continue
			}
			available[balance.AccountID] = balance.Balance
		}
		for _, account := range accounts {
			accountIDByKey[newAccountKey(account.AccountType, account.OwnerUserID, account.Asset)] = account.ID
		}
		...
		for _, i := range activeOwners {
			order := reqs[i].order
			owner := order.UserID
			accountID, ok := accountIDByKey[newAccountKey(model.AccountUserAvailable, &owner, orderHoldAsset(order))]
			if !ok {
