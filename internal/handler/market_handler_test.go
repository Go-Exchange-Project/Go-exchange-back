...
func TestMarketRulesResponseUsesDecimalStringsAndOpenEndedFinalTick(t *testing.T) {
	...
	assert.Equal(t, "0", response.MinOrderNotional)
	...
	assert.Equal(t, "0.00001", *response.TickRules[0].UpperBound)
	assert.Equal(t, "0.00000001", response.TickRules[0].TickSize)
...
func TestMarketHandlerGetRules(t *testing.T) {
	...
	assert.Equal(t, "0", body.MinOrderNotional)
	...
	require.Len(t, body.TickRules, 16)
}
